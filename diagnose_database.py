...
def diagnose():
    ...
    masked_url = db_url.replace(db_url.split("@")[0] if "@" in db_url else "", "[CREDENCIAIS]")
    ...
    if not motoristas:
        ...
        for m in motoristas[:5]:  # Mostrar apenas os 5 primeiros
            ...
            print(f"   {status_icon} {m.nome:25} | Tel: {m.telefone:15} | Plano: {m.plano}")
        if len(motoristas) > 5:
    ...
    if not servicos:
        ...
        for s in servicos[:5]:
            print(f"   🛣️  {s.nome:30} → R$ {s.valor_base:8.2f}")
        if len(servicos) > 5:
