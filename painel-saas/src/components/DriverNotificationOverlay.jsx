...
const DriverNotificationOverlay = ({ motoristaId }) => {
  ...
  const aceitarPedido = async () => {
    try {
      ...
      alert("Sucesso! O pedido agora é seu.");
      limparNotificacao();
    } catch (error) {
      ...
      <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-bounce">
        <div className="text-center">
          ...
          <div className="flex flex-col gap-3">
            <button onClick={aceitarPedido} className="bg-indigo-600 text-white font-bold py-4 rounded-xl hover:bg-indigo-700 transition">
              ACEITAR AGORA
            </button>
            <button onClick={limparNotificacao} className="text-gray-400 font-medium py-2">
              Recusar
            </button>
          </div>
        </div>
